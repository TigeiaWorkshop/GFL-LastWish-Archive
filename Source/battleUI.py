...
class SelectMenu:
    ...
    def display(self,screen,fontSize,location,kind,friendsCanSave,thingsCanReact):
        ...
        if linpg.ifHover(self.allButton["attack"],(txt_tempX,txt_tempY)):
        ...
        #txt_tempY 与攻击按钮一致
        if linpg.ifHover(self.allButton["move"],(txt_tempX,txt_tempY)):
            buttonGetHover = "move"
        ...
        if linpg.ifHover(self.allButton["reload"],(txt_tempX,txt_tempY)):
        ...
        if kind != "HOC":
            ...
            if linpg.ifHover(self.allButton["skill"],(txt_tempX,txt_tempY)):
        ...
        if len(friendsCanSave)>0:
            txt_tempX = location["xStart"]-selectButtonBaseWidth*0.6
            txt_tempY = location["yStart"]-selectButtonBaseWidth*0.7
            if linpg.ifHover(self.allButton["rescue"],(txt_tempX,txt_tempY)):
        ...
        if len(thingsCanReact)>0:
            txt_tempX = location["xEnd"]-selectButtonBaseWidth*0.4
            txt_tempY = location["yStart"]-selectButtonBaseWidth*0.7
            if linpg.ifHover(self.allButton["interact"],(txt_tempX,txt_tempY)):
