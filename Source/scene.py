...
def dialogCreator(chapterType,chapterId,screen,part,collection_name=None):
    ...
    Zero.unloadBackgroundMusic()
    #加载对话
    DIALOG = Zero.DialogSystemDev(chapterType,chapterId,part,collection_name)
    #主循环
    while True:
        if not DIALOG.display(screen):
            Zero.display.flip()
...
def mapCreator(chapterType,chapterId,screen,collection_name=None):
    #卸载音乐
    Zero.unloadBackgroundMusic()
    MAPCREATOR = Zero.mapCreator(chapterType,chapterId,collection_name)
